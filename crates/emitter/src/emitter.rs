...
impl Emitter {
    ...
    // ----- Doc IR helpers --------------------------------------------------
    //
    // Walker methods use these to build explicit Group / nest / soft-line
    // structure for Wadler-style width-aware layout. Sub-buffer helpers
    // (`buf_begin` / `buf_end_concat`) capture a chunk of children so the
    // caller can wrap them (e.g. in `Doc::ForceFlat` for compact attrs).
    ...
    /// Outer break for a list-like construct. Emits `UserBreak` when the
    /// user broke the surrounding pair across lines, otherwise a soft
    /// break. No-op in `Mode::Align`.
...
impl VerylWalker for Emitter {
    ...
    fn struct_constructor_list(&mut self, arg: &StructConstructorList) {
        ...
        for x in &arg.struct_constructor_list_list {
            ...
            // Fill mode: short lists stay flat while long ones break
            // per-item. UserBreak preserves the source's line layout when
            // the user explicitly broke between items.
            self.group_begin();
            self.comma(&x.comma);
            if item_user_broken {
                self.emit_doc(Doc::UserBreak);
    ...
    fn concatenation_list(&mut self, arg: &ConcatenationList) {
        let user_broken = arg
            ...
            .iter()
            .any(|x| x.comma.line() != x.concatenation_item.line());
    ...
    fn array_literal_list(&mut self, arg: &ArrayLiteralList) {
        let user_broken = arg
            ...
            .iter()
            .any(|x| x.comma.line() != x.array_literal_item.line());
    ...
    fn attribute(&mut self, arg: &Attribute) {
        ...
        match identifier.as_str() {
            "ifdef" | "ifndef" | "elsif" | "else" => {
                ...
                // Inspect the trailing Doc nodes to swallow a preceding
                // `endif` pair when this attribute is `elsif` / `else`. A
                // freshly emitted `endif` is the pair
                // `[Hardline, Doc::Text("`endif")]`; the list iterator may
                // add a trailing `Hardline`, so the canonical pattern is
                // `[Hardline, Text("`endif"), Hardline]`. We drop the inner
                // `endif` pair (and re-emit a single `Hardline` if the
                // pattern was incomplete) so the `elsif` lands on a new
                // line directly following the previous branch's body.
